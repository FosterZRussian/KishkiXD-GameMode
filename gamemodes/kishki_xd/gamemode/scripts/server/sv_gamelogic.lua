...
util.AddNetworkString("FosterZ_ItemCode")
...
function SOCIOPATHY_PROJECT.GameLogic:SetupPlayerToStart(ply, model, gm_id)
	...
	local gm_table = SOCIOPATHY_PROJECT.CONFIG.GAMEMODES:GetGameModeTable(gm_id)
	local give_list = gm_table.ItemsOnGiveList

	for k,v in pairs(gm_table.DefaultItemsOnGiveList) do
		SOCIOPATHY_PROJECT.GameLogic:GiveItemToPlayer(ply, k)	
	end

	local items = table.GetKeys(give_list)	

	local id, key = table.Random(items)
	
	if id != nil then
		SOCIOPATHY_PROJECT.GameLogic:GiveItemToPlayer(ply, id)
	end
		--[[ 
	items[key] = nil
	id, key = table.Random(items)
	SOCIOPATHY_PROJECT.GameLogic:GiveItemToPlayer(ply, id)
	items[key] = nil--]] 
...
function SOCIOPATHY_PROJECT.GameLogic:SetupGameWorld(gm_id, is_record)

	local gm_data = SOCIOPATHY_PROJECT.CONFIG.GAMEMODES:GetGameModeTable(gm_id)
	game.GetWorld():SetNWString("GameModeID", gm_id) --<<<<<<<<<<


	game.GetWorld():SetNWBool("IsRecord_Mode", true)

	game.CleanUpMap()

	SOCIOPATHY_PROJECT.GameLogic.ItemsNavAreas = {}
	...
	gm_data:OnGameModeStart(is_record)
	return true
...
net.Receive("FosterZ_StartGame", function(len, ply)
	local gm_name = net.ReadString()
	local is_record = net.ReadBool()
	local model = net.ReadString()
	--local gm_id = net.ReadUInt(32)


	if SOCIOPATHY_PROJECT.GameLogic:SetupGameWorld(gm_name, is_record) then
		SOCIOPATHY_PROJECT.GameLogic:SetupPlayerToStart(ply, model, gm_name)
	end
...
net.Receive("FosterZ_ItemCode", function(len,ply)
	local code = net.ReadString()
	local weapon, weapon_id = SOCIOPATHY_PROJECT.GameLogic:GetPlayerActiveItem(ply)
	if weapon != nil then		
		if weapon.ItemData.SystemFunctions.OnNetCode[code] != nil then			
			weapon.ItemData.SystemFunctions.OnNetCode[code](weapon)	
		end
	end	
end)
