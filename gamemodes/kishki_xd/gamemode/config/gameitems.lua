...
local defaultItemData = {
	ItemData = {
		...
		SystemFunctions = {
			...
			ScreenDraw_PreDraw = function(self, item_config, w,h)
			end,
			ScreenDraw_PreDraw3D = function(self, item_config, w,h)
			end,
...
for k,v in pairs(file.Find("gamemodes/kishki_xd/gamemode/config/gameitems/*.lua", "GAME")) do
	local item_name = string.sub(v, 1, string.len(v)-4)
	SOCIOPATHY_PROJECT.GameLogic:IncludeItem(item_name)
end
for k,v in pairs(file.Find("gamemodes/kishki_xd/gamemode/config/gameitems/visual_items/*.lua", "GAME")) do
	local item_name = string.sub(v, 1, string.len(v)-4)
	SOCIOPATHY_PROJECT.GameLogic:IncludeItem("visual_items/"..item_name)
end
